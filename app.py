...
from exercises.exercise_4_audit import build_graph, resume_command
...
async def _recent_sessions(limit: int = 25) -> list[dict]:
...
with st.sidebar:
    ...
    try:
        ...
        sessions = []
        st.caption(f"Could not load sessions: {exc}")

    if not sessions:
        st.caption("No recorded sessions yet.")
    for idx, session in enumerate(sessions):
        label = f"{session['worst_risk']} · {session['pr_url']} · {session['last_event']}"
        if st.button(label, key=f"session-{idx}", use_container_width=True):
...
async def run_graph(pr_url: str, thread_id: str, resume_value=None):
    ...
    async with AsyncSqliteSaver.from_conn_string(db_path()) as cp:
        ...
        return await app.ainvoke(resume_command(interrupt_payload, resume_value), cfg)
