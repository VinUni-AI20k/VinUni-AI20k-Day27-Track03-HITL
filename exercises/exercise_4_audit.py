...
def resume_command(payload: dict, answer):
    kind = payload["kind"]
    updates = {}
    if kind == "approval_request":
        updates["_human_approval_prompt_logged"] = True
    elif kind == "escalation":
        updates["_escalation_prompt_logged"] = True
    return Command(resume=answer, update=updates)
        ...
        a: PRAnalysis = await llm.ainvoke([
            {
                "role": "system",
                "content": (
                    "Senior reviewer. Structured output. If confidence is below 0.60, "
                    "populate escalation_questions with 2-4 specific, context-rich "
                    "questions that reference the file or code area in the diff where "
                    "you are uncertain."
                ),
            },
...
async def node_human_approval(state):
    ...
    if not state.get("_human_approval_prompt_logged"):
        await audit(state, AuditEntry(
            agent_id=AGENT_ID,
    ...
    await audit(state, AuditEntry(
        ...
        reason=resp.get("feedback") or a.confidence_reasoning,
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    return {
        "human_choice": resp.get("choice"),
        "human_feedback": resp.get("feedback"),
        "_human_approval_prompt_logged": False,
    }
...
async def node_commit(state):
    ...
    if state.get("escalation_answers"):
        ...
        audit_decision = state["human_choice"]
    elif action == "rejected":
        audit_decision = "reject"
    else:
        audit_decision = "pending"
    reason = state.get("human_feedback")
    if state.get("escalation_answers"):
        reason = f"Committed after escalation with {len(state['escalation_answers'])} answer(s)"
    elif action == "committed":
        reason = reason or "Reviewer approved and comment was posted"
    elif action == "rejected":
        reason = reason or "Reviewer did not approve"
    else:
        reason = reason or "Posting review comment failed"
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="commit",
        confidence=a.confidence,
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=a.confidence_reasoning if action == "committed" else "Auto-approval post failed",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    if not state.get("_escalation_prompt_logged"):
        await audit(state, AuditEntry(
            agent_id=AGENT_ID,
            action="escalate",
            confidence=a.confidence,
    ...
    return {"escalation_answers": answers, "_escalation_prompt_logged": False}
...
async def run(pr_url: str, thread_id: str | None):
    ...
    async with AsyncSqliteSaver.from_conn_string(db_path()) as cp:
        ...
        while "__interrupt__" in result:
            ...
            result = await app.ainvoke(resume_command(payload, answer), cfg)
