    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "specific, context-rich questions that reference the file or code area "
                "in the diff where you are uncertain."
            )},
        ...
        refined = llm.invoke([
            {
                "role": "system",
                "content": (
                    "Refine the pull request review using the original analysis and the "
                    "reviewer's answers. Update confidence, comments, and summary."
                ),
            },
            {
                "role": "user",
                "content": (
                    ...
                    f"Diff:\n{state['pr_diff']}\n\n"
                    f"Initial summary:\n{initial.summary}\n\n"
                    f"Initial confidence reasoning:\n{initial.confidence_reasoning}\n\n"
                    f"Initial risk factors:\n" + "\n".join(f"- {item}" for item in initial.risk_factors) + "\n\n"
                    f"Initial comments:\n" + "\n".join(
